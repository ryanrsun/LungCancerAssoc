...
#' @param part This is the difference between run_pathwayanal.R and run_pathwayanal_partial.R
#' This should be a single integer between 1-pathways_per_job, and it will only do that element
#' of the pathways list AFTER it has already been cut by pathways_per_job. Sometimes you specify
#' too many pathways_per_job and you need to make it smaller, but you don't want to mess up the
#' naming conventions of already finished jobs.  The output will be named
#' [out_name_root]_S[Snum]_[aID]_part[part].txt.
...
run_pathwayanal_part <- function(part=NULL, pathways_tab=NULL, pathways_tab_fname=NULL,
    ...
    # The one difference with run_pathwayanal.R - check for part
    if (!is.numeric(part)) {
        stop('part variable must be an integer between 1-pathways_per_job')
    }
    if (part < 1 | part > pathways_per_job) {
        stop('part variable must be an integer between 1-pathways_per_job')
    }
    pathways_tab <- pathways_tab[part, ]
    ...
    # Write - change outname for part
    setwd(output_dir)
    out_name <- paste(out_name_root, '_S', Snum, '_', aID, '_part', part, '.txt', sep='')
